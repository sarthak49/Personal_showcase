...
speechRs.speechinit = function (lang, cb, bcolor, color, pitch, rate) {
  ...
  setTimeout(function () {
    speechRs.speaker.voice = speechSynthesis
      .getVoices()
      .filter(function (voice) {
        return voice.name == speechRs.lan;
      })[0];
  }, 500);
  if (lang == "native") {
    cb(this);
...
speechRs.speak = function (text, cb, isHiligh) {
  let j = 0,
    el,
    ar = [];
  speechRs.speaker.voice = speechSynthesis.getVoices().filter(function (voice) {
    return voice.name == speechRs.lan;
  })[0];
...
speechRs.rec_start = function (l, callback) {
  ...
  this.recognition.onresult = function (event) {
    ...
    callback(
      speechRs.final_transcript.replace("undefined", ""),
      interim_transcript
    );
  };
};
